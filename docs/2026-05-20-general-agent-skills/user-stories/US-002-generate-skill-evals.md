...
Scenario: Generate evals for each skill
  ...
  Then each new skill has evals/evals.yaml
...
5. **Validate YAML:** Ensure every `evals.yaml` file is valid YAML and follows the local create-skill expectations.
...
- [ ] **Compilation:** All new `evals/evals.yaml` files parse as valid YAML.
