...
- **Epic Context:** Implements the approved General Agent Skills PRD/SPEC by creating `ask`, `reason`, `classify`, `plan`, `explore`, `decide`, `coordinate`, and `remember`. Existing `explain` is already complete and must not be rewritten unless validation reveals a spec violation.
...
- **In-Scope:**
  - Create `.agents/skills/<skill-name>/SKILL.md` for `ask`, `reason`, `classify`, `plan`, `explore`, `decide`, `coordinate`, and `remember`.
...
- **Primary Target Files:**
  ...
  2. `.agents/skills/reason/SKILL.md` -> New ambiguous-problem reasoning skill.
  ...
  6. `.agents/skills/decide/SKILL.md` -> New decision support skill.
  7. `.agents/skills/coordinate/SKILL.md` -> New coordination skill.
...
1. **Analyze & Validate:** Read [SPEC.md](../SPEC.md), `.agents/skills/creator-skill/SKILL.md`, and `.agents/skills/creator-skill/references/authoring.md`.
