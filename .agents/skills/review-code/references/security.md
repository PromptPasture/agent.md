...
**A security finding should describe the trust boundary, attacker capability, vulnerable path, and impact.**
...
**Check whether every changed action is authorized at the right boundary.**
...
**Check untrusted input at every parser, interpreter, and renderer.**
...
**Check that sensitive data cannot leak through normal or failure paths.**
...
**Check session and trust-boundary controls where state or callers change.**
...
**Check model-mediated paths as untrusted input and action surfaces.**
