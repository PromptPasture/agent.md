...
There is no global install step for this .agents directory. Use a skill by copying or referencing its folder from the agent/runtime you are using.
...
```bash
python3 .agents/skills/creator-skill/scripts/quick_validate.py .agents/skills/my-skill
```

If `python` is not found, use `python3`.

If module commands fail with `No module named scripts`, run them from `.agents/skills/creator-skill` with `python3 -m ...`.
